#!/usr/bin/env python3
"""
Multi-Agent Freeciv — multiple LLM agents playing as different players.
...
from agent import FreecivAgent, DEFAULT_SYSTEM_PROMPT

logger = logging.getLogger("freeciv-multi")

STRATEGY_PROMPT_TEMPLATE = """You are an expert Freeciv player AI agent named "{name}". You control a civilization and make strategic decisions each turn.
...
class GameOrchestrator:
    def __init__(self):
        self.agents: dict[str, FreecivAgent] = {}
    ...
    async def create_game(self, agent_configs: list[dict],
        ...
        for cfg in agent_configs:
            ...
            agent = FreecivAgent(client, name=cfg["name"],
    ...
    def get_agent(self, name: str) -> FreecivAgent:
...
app = FastAPI(title="Freeciv Multi-Agent API", lifespan=lifespan)
...
async def main():
    parser = argparse.ArgumentParser(description="Freeciv Multi-Agent")
